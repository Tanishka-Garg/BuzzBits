...
export class News extends Component {
    ...
    return (
        <>
            <h1 className='container' style={{marginTop:"70px"}}> Top Buzz  <small className="text-muted">{this.props.category}</small></h1>
                ...
                {this.state.articles.map((element)=>{
                    return <div className="col-md-4 my-3"  key ={element.url}>
                        <Newsitem title={element.title ? element.title.slice(0, 40) : ""} description={element.description ? element.description.slice(0, 80) : ""}
                            date={element.publishedAt ? element.publishedAt.substring(0,10)+" "+element.publishedAt.substring(11,19): ""}
